...
```bash
# macOS / Linux
bash .initium/sync.sh --check    # güncelleme mevcut mu kontrol et
bash .initium/sync.sh            # güncellemeleri etkileşimli olarak uygula
```
...
Senkronizasyon betiği proje özgü dosyalarına (`CLAUDE.md`, `docs/context/`, `agent.config.yaml`) asla dokunmaz. Ayrıntılar için: [`.initium/sync-guide.md`](initium-sync.md)

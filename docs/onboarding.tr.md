...
bash scripts/validate-ai-config.sh   # beklenen: tüm PASS, FAIL yok
```
...
# --- İskelet bakımı ---
/sync-skeleton — üst iskelet yeni geliştirmelerini çek
```
...
```bash
# macOS / Linux
bash scripts/sync-skeleton.sh --check    # güncelleme mevcut mu kontrol et
bash scripts/sync-skeleton.sh            # güncellemeleri etkileşimli olarak uygula
```
...
Senkronizasyon betiği proje özgü dosyalarına (`CLAUDE.md`, `docs/context/`, `agent.config.yaml`) asla dokunmaz. Ayrıntılar için: [`docs/skeleton-sync.md`](skeleton-sync.md)

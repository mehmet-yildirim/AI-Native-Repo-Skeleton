...
```bash
# macOS / Linux
bash scripts/sync-initium.sh --check    # güncelleme mevcut mu kontrol et
bash scripts/sync-initium.sh            # güncellemeleri etkileşimli olarak uygula
```
...
Senkronizasyon betiği proje özgü dosyalarına (`CLAUDE.md`, `docs/context/`, `agent.config.yaml`) asla dokunmaz. Ayrıntılar için: [`docs/initium-sync.md`](initium-sync.md)
